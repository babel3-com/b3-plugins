...
**Why dangerous:** A buffer size constant in the client depends on a body size limit in nginx depends on a timeout in Cloudflare. Change one without updating the others and the system breaks — but only under load, making it hard to test.
